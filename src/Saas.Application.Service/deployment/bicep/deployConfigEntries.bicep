...
@description('Application ID URI for the exposed App API scopes.')
param applicationIdUri string

@description('App API scopes.')
param appApiScopes string
...
var appConfigStore = {
  ...
  entries: [
    ...
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:BaseUrl'
      value: baseUrl
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:ClientId'
      value: clientId
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:TenantId'
      value: azureB2cTenantId
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:Domain'
      value: azureB2CDomain
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:Instance'
      value: azureAdB2CInstanceURL
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:Audience'
      value: clientId
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:SignedOutCallbackPath'
      value: signedOutCallBackPath
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:${azureB2CKeyName}:SignUpSignInPolicyId'
    ...
    {
      key: '${appApiKeyName}:ApplicationIdUri'
      value: applicationIdUri
      isSecret: false
      contentType: 'text/plain'
    }
    {
      key: '${appApiKeyName}:Scopes'
      value: ' ${string(appApiScopes)}' // notice the space before the string, this is a necessary hack. https://github.com/Azure/bicep/issues/6167
      isSecret: false
